...
func main() {
	...
	if cfg.db.dsn == "" {
		cfg.db.dsn = "postgres://myuser:secret@localhost/greenlight?sslmode=disable"
	}
	...
	// --- Logger ---
	// Initialize a new logger wich writes messages to the standard out stream,
	...
	// Declare a instance of the application struct, containing the config struct and
	// the logger
	app := &application{
		config: cfg,
		logger: logger,
	...
	// Declare a HTTP server with some sensible timeout settings, wich listens on the
	// port provided in the config struct and uses the servemux we created aborve as the handler
	srv := &http.Server{
